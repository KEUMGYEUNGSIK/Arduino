...
} aa, m_robot_angle,crc_check_sum;
			...
			if(read_buf[cnt]=='*')
			{
				if(read_buf[cnt-8]=='#')
				{
				    if(read_buf[cnt-7]=='I'||read_buf[cnt-7]=='F')
				    {
						...
						{
							crc_check_sum.data += read_buf[i];
						}
						
						if(crc_check_sum.bytedata[0] == read_buf[cnt-1] && crc_check_sum.bytedata[1] == read_buf[cnt-2])
						{
							...
							aa.bytedata[1]=read_buf[cnt-4];
							aa.bytedata[0]=read_buf[cnt-3];

							aa.data = (3.3/4095) * aa.data;
			...
			{		
				if(read_buf[cnt-total_val]=='#')
				{
					if(read_buf[cnt-(total_val-1)]=='I')
					{
						...
						{
							printf("perfect! %d \b %d",read_buf[cnt-1],read_buf[cnt-2]);
							printf("\n");
							...
							aa.bytedata[0] = read_buf[cnt-3];
							aa.bytedata[1] = read_buf[cnt-4];
							//aa.bytedata[2] = read_buf[cnt-5];
							//aa.bytedata[3] = read_buf[cnt-6];
